...
Status calculate(const Stack eq, ElemType* carrier, const int singal) {
	...
	for(i = 0; i < eq->length; ++i) {
		...
		if(inOperSet(temp)) {
			...
			if(isEmptyStack(operS)) push(operS,index);
			else {
				int flag = 0;
				do {
					if(isEmptyStack(operS)) {
						if(!push(operS,index)) return ERROR;
						break;
					}
					int type;
					flag = 0;
					if(!getTop(operS,&type)) return ERROR;
					if((type / 2) >= (index / 2)) {
						pop(operS,&type);
						...
						pop(numS,&num1);
						pop(numS,&num2);
						if(!push(numS,operater(num1,num2,type))) return ERROR;
						flag = 1;
					} else push(operS,index);
				} while(flag);
			}
		} else {
			int res = 0;
			do {
				temp = eq->data[i];
				res = res * 10 + (temp - '0');
				if(inOperSet(eq->data[i+1]) || i+1 == eq->length) break;
				++i;
			} while(i < eq->length);
			push(numS,res);
		}
		if (singal) showProcess(numS,operS);
	}
