...
Status minRoute(Graph graph, Graph other)
{
	...
	printf("StartNode >> ");
	scanf("%d", &start);
	if (start < 0 || start > NODENUM - 1)
		return ERROR;
	printf("EndNode >> ");
	scanf("%d", &end);
	if (end < 0 || end > NODENUM - 1)
		return ERROR;
	if (!dijkstra(graph, start, end, other))
		return ERROR;
	return OK;
}
...
Status minTree(Graph graph)
{
	...
	printf("BeginNode >> ");
	scanf("%d", &begin);
	if (begin < 0 || begin > NODENUM - 1)
		return ERROR;
	if (!prime(graph, begin))
		return ERROR;
	return OK;
}
...
Status parseRouteInfo(char *phrase, Graph *distanceGraph, Graph *priceGraph)
{
	...
	if (!(addEdge(priceGraph, startIndex, endIndex, price)))
		return ERROR;

	startIndex = getNodeIndex(distanceGraph, startPlace);
	endIndex = getNodeIndex(distanceGraph, endPlace);
	if (!(addEdge(distanceGraph, startIndex, endIndex, distance)))
		return ERROR;
